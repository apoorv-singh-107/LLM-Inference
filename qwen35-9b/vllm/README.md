# Qwen-3.5-9B with vLLM

This directory contains the necessary configurations to deploy the `Qwen/Qwen3.5-9B` model using `vllm` as the serving backend.
...
4.  **vLLM Server**: The vLLM server runs the `Qwen/Qwen3.5-9B` model, performs the inference, and streams the response back to the gateway.
...
-   **vLLM Server (`vllm/vllm-openai:latest`)**: The core inference engine. It uses the `vllm` library to serve the `Qwen/Qwen3.5-9B` model, optimized for high-throughput and low-latency.
...
-   **`vllm-qwen35-9b` service**:
    -   `image: vllm/vllm-openai:latest`: Uses the latest official vLLM OpenAI-compatible server image.
    -   `gpus: all`:  Assigns all available GPUs to the container.
    -   `environment`: Passes the Hugging Face token and other PyTorch specific environment variables.
    -   `volumes`: Mounts the local Hugging Face cache to speed up model loading on subsequent runs.
    -   `command`: Specifies the model to load (`Qwen/Qwen3.5-9B`) and other vLLM server parameters. It also enables speculative decoding.

-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - vllm-qwen35-9b`: Ensures that the vLLM server is started before the gateway.
-   **`nginx` service**:
...
-   **`MODEL_MAP`**: A dictionary that maps model names to their corresponding vLLM server URLs.
...
This configuration uses speculative decoding to accelerate inference.

-   `--speculative-config '{"method": "mtp", "num_speculative_tokens": 1}'`: Enables speculative decoding with the `mtp` method.
