# Ministral-3B-Instruct with vLLM

This directory contains the necessary configurations to deploy the `ministral3-3b-inst` model using `vllm` as the serving backend.
...
4.  **vLLM Server**: The vLLM server runs the `ministral3-3b-inst` model, performs the inference, and streams the response back to the gateway.
...
-   **vLLM Server (`vllm/vllm-openai:latest`)**: The core inference engine. It uses the `vllm` library to serve the `ministral3-3b-inst` model, optimized for high-throughput and low-latency.
...
-   **`vllm-ministral3-3b-inst` service**:
    ...
    -   `command`: Specifies the model to load (`mistralai/Ministral-3-3B-Instruct-2512`) and other vLLM server parameters, such as `gpu-memory-utilization`, `max-model-len`, etc.
-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - vllm-ministral3-3b-inst`: Ensures that the vLLM server is started before the gateway.
-   **`nginx` service**:
...
-   **`../.env`**: You must provide your Hugging Face token in the `.env` file in the parent directory to allow the vLLM server to download the model from the Hugging Face Hub.
