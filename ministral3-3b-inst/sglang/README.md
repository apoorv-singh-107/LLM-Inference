# Ministral-3 3B Instruct (`ministral3-3b-inst`) with SGLang

This directory contains the necessary configurations to deploy the `mistralai/Ministral-3-3B-Instruct-2512` model using `sglang` for serving, with an NGINX reverse proxy and a FastAPI gateway.
...
4.  **SGLang Server**: The SGLang server runs the `mistralai/Ministral-3-3B-Instruct-2512` model, performs the inference, and streams the response back to the gateway.
...
-   **SGLang Server (`lmsysorg/sglang:latest`)**: The core inference engine. It uses the `sglang` library to serve the `mistralai/Ministral-3-3B-Instruct-2512` model, optimized for high-throughput and low-latency.
...
-   **`sglang-ministral3-3b-inst` service**:
    ...
    -   `environment`: Passes the Hugging Face token and other SGLang-related environment variables from the `.env` file.
    -   `volumes`: Mounts the local Hugging Face cache to speed up model loading on subsequent runs.
    -   `command`: Specifies the model to load (`mistralai/Ministral-3-3B-Instruct-2512`) and other SGLang server parameters.
-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - sglang-ministral3-3b-inst`: Ensures that the SGLang server is started before the gateway.
-   **`nginx` service**:
