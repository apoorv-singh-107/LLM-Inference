# NVIDIA-Nemotron-3-Nano-4B-BF16 with SGLang

This directory contains the necessary configurations to deploy the `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` model using `sglang` as the serving backend.
...
4.  **SGLang Server**: The SGLang server runs the `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` model, performs the inference, and streams the response back to the gateway.
...
-   **SGLang Server (`lmsysorg/sglang:latest`)**: The core inference engine. It uses the `sglang` library to serve the `nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16` model.
...
-   **`sglang-nemotron3-nano-4b` service**:
    ...
    -   `command`: Specifies the model to load (`nvidia/NVIDIA-Nemotron-3-Nano-4B-BF16`) and other SGLang server parameters.
-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - sglang-nemotron3-nano-4b`: Ensures that the SGLang server is started before the gateway.
-   **`nginx` service**:
...
-   **`MODEL_MAP`**: A dictionary that maps model names to their corresponding SGLang server URLs. The key is the full Hugging Face model ID.
