# Qwen-3.5-0.8B with SGLang

This directory contains the necessary configurations to deploy the `Qwen/Qwen3.5-0.8B` model using `sglang` as the serving backend.
...
4.  **SGLang Server**: The SGLang server runs the `Qwen/Qwen3.5-0.8B` model, performs the inference, and streams the response back to the gateway.
...
-   **SGLang Server (`lmsysorg/sglang:latest`)**: The core inference engine. It uses the `sglang` library to serve the `Qwen/Qwen3.5-0.8B` model, optimized for high-throughput and low-latency.
...
-   **`sglang-qwen35-0.5b` service**:
    ...
    -   `command`: Specifies the model to load (`Qwen/Qwen3.5-0.8B`) and enables speculative decoding with the EAGLE algorithm.

-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - sglang-qwen35-0.5b`: Ensures that the SGLang server is started before the gateway.
-   **`nginx` service**:
