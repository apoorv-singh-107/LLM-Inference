# Qwen-3.5-2B with vLLM

This directory contains the necessary configurations to deploy the `Qwen/Qwen3.5-2B` model using `vllm` as the serving backend.
...
4.  **vLLM Server**: The vLLM server runs the `Qwen/Qwen3.5-2B` model, performs the inference, and streams the response back to the gateway.
...
-   **vLLM Server (`vllm/vllm-openai:v0.17.0`)**: The core inference engine. It uses the `vllm` library to serve the `Qwen/Qwen3.5-2B` model, optimized for high-throughput and low-latency.
...
-   **`vllm-qwen35-2b` service**:
    ...
    -   `command`: Specifies the model to load (`Qwen/Qwen3.5-2B`) and other vLLM server parameters.
-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - vllm-qwen35-2b`: Ensures that the vLLM server is started before the gateway.
-   **`nginx` service**:
...
-   **`MODEL_MAP`**: A dictionary that maps model names to their corresponding vLLM server URLs. This has been corrected to point to the correct service: `{"Qwen/Qwen3.5-2B": "http://vllm-qwen35-2b:8000"}`.
