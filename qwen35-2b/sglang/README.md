# Qwen-3.5-2B with SGLang

This directory contains the necessary configurations to deploy the `Qwen/Qwen3.5-2B` model using `sglang` as the serving backend.
...
4.  **SGLang Server**: The SGLang server runs the `Qwen/Qwen3.5-2B` model, performs the inference, and streams the response back to the gateway.
...
-   **SGLang Server (`lmsysorg/sglang:latest`)**: The core inference engine. It uses the `sglang` library to serve the `Qwen/Qwen3.5-2B` model, optimized for high-throughput and low-latency.
...
-   **`sglang-qwen35-2B` service**:
    ...
    -   `command`: Specifies the model to load (`Qwen/Qwen3.5-2B`) and enables speculative decoding with the EAGLE algorithm.

-   **`gateway` service**:
    -   `build: ./gateway`: Builds the FastAPI gateway image from the `gateway` directory.
    -   `depends_on: - sglang-qwen35-2B`: Ensures that the SGLang server is started before the gateway.
-   **`nginx` service**:
...
-   **`MODEL_MAP`**: A dictionary that maps model names to their corresponding SGLang server URLs. This has been corrected to point to the correct service.
-   **`DEFAULT_MODEL`**: The default model to be used if the request does not specify a model. This has been corrected to `Qwen/Qwen3.5-2B`.
